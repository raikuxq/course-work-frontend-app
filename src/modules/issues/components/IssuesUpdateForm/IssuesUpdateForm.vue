...
import type {T_GQL_issueReport_issueReport, T_GQL_tracker_tracker_members_user} from "@/shared/types/graphql";
...
const submit = async (event) => {
  ...
  try {
    ...
    if (data) {
      message.success(t('issues.notify.updated'))
      emit('updateData')
      emit('closeModal')
    }
  } catch (error) {
    ...
    if (alertMessage) {
      message.error(`${t('issues.notify.not_updated')}: ${error.message}`);
    }
  }
}
</script>
