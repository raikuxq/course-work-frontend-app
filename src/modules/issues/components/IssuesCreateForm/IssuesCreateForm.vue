...
type TProfileChangePasswordPropsMember = {
  id: string;
...
type TProfileChangePasswordProps = {
  isModalOpen: boolean;
  trackerId: string;
  members: TProfileChangePasswordPropsMember[];
}
...
const submit = async (event) => {
  ...
  try {
    ...
    if (data) {
      message.success(t('bug.form.success'))
      emit('updateData')
      emit('closeModal')
    }
  } catch (error) {
    ...
    if (alertMessage) {
      message.error(`${t('bug.form.success')}: ${error.message}`);
    }
  }
}
</script>
